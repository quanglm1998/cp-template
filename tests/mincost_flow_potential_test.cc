...
#include <../src/mincost_flow_potential.h>
#include <gtest/gtest.h>

mt19937 rng;

TEST(MincostflowTest, Simple) {
  MinCostFlowPotential<int> g(4, 0, 3);
  g.AddEdge(0, 1, 1, 1);
  g.AddEdge(0, 2, 1, 1);
  g.AddEdge(1, 3, 1, 1);
  g.AddEdge(2, 3, 1, 1);
...
TEST(MincostflowTest, Stress) {
  for (int phase = 0; phase < 1000; phase++) {
    ...
    MinCostFlowPotential<int> g(n, s, t);
...
TEST(DinicFlowTest, Set) {
  MinCostFlowPotential<int> g(100, 0, 1);
